...
export function summarizeToolOutput(output: unknown, ar: boolean): string {
  ...
  if (typeof o.summary === "string") return o.summary.slice(0, 220);
...
export function extractDocumentPreview(output: unknown): {
  ...
  if (agentStates && agentStates.length) {
    const done = agentStates.filter((a) => a.status === "completed" || a.status === "done").length;
    const progress = done / agentStates.length;
    return {
      title: title || "Agent pipeline",
      body: agentStates
        .map((a) => `${a.name ?? "agent"}: ${a.status ?? "…"} (${Math.round((a.progress ?? 0) * 100)}%)`)
        .join("\n"),
      progress,
