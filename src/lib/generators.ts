  ...
  const labels =
    locale === "ar"
      ? {
          ...
          // Qualified as a category default. These come from the static
          // TENDER_TYPES table, not from the tender document: the ingestion
          // agent can detect real weights but they are not persisted on the
          // project, so the generator has nothing else to print. Stating them
          // unqualified in a document submitted to a government buyer asserts
          // terms the platform never read.
          techFin: `فني ${tenderType.evaluationSplit.technical}% / مالي ${tenderType.evaluationSplit.financial}% (افتراضي الفئة)`,
          slaVal: `${tenderType.slaPerWeek}% أسبوعياً (حد أقصى ${tenderType.slaMaxPenalty}%) (افتراضي الفئة)`,
        }
      : {
          ...
          techFin: `Technical ${tenderType.evaluationSplit.technical}% / Financial ${tenderType.evaluationSplit.financial}% (category default)`,
          slaVal: `${tenderType.slaPerWeek}%/week (max ${tenderType.slaMaxPenalty}%) (category default)`,
        };
