...
export function MissionStage({
  ...
  return (
    <div className="flex min-h-0 w-full min-w-0 max-w-full flex-1 flex-col gap-2.5 sm:gap-3">
      <MissionActionTicker locale={locale} tools={tools} listening={listening} speaking={speaking} thinking={thinking} offline={offline} />
      ...
      <div className="grid min-h-0 w-full min-w-0 max-w-full flex-1 gap-3 lg:grid-cols-[minmax(0,1.15fr)_minmax(300px,0.85fr)] lg:gap-3.5 items-start lg:items-stretch">
        ...
        <div className={cn("min-h-0 w-full min-w-0 max-w-full", tab === "activity" ? "block" : "hidden lg:block")}>
          <div className="flex h-full min-h-0 flex-col gap-2">
            ...
            {/*
              No `min-h` floor at lg. `MissionControlShell` caps the page at
              `calc(100dvh-6.5rem)` and clips the overflow, so a 560px floor
              inside a flex column that has to shrink meant this panel and the
              composer above it got cut off rather than scrolled. It fills the
              grid row now and scrolls itself.
            */}
            <MissionToolTheater locale={locale} tools={tools} voiceLive={voiceLive} isCapturing={listening} isSpeaking={speaking} className="min-h-[360px] max-h-[min(78dvh,860px)] lg:h-full lg:min-h-0 lg:max-h-none overflow-y-auto overscroll-contain scrollbar-thin scrollbar-thumb-zinc-300 dark:scrollbar-thumb-white/10" />
          </div>
        </div>
      </div>
    </div>
