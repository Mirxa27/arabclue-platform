...
export function ReviewsQueue() {
  ...
  const decide = useMutation({
    mutationFn: async (opts: { id: string; status: "APPROVED" | "REJECTED" }) => {
    ...
    onSuccess: () => {
      ...
      toast({ title: locale === "ar" ? "تم تسجيل القرار" : "Decision recorded" });
    },
  ...
  return (
    <Panel
      ...
      subtitle={locale === "ar" ? "العروض بانتظار قرارك" : "Proposals awaiting your decision"}
    >
      {isLoading ? (
        ...
        <p className="p-6 text-sm text-muted-foreground">
          {locale === "ar" ? "لا توجد مراجعات معلقة." : "No pending reviews."}
        </p>
      ) : (
        <ul className="divide-y">
          {(data?.items ?? []).map(
            (r: {
              ...
              stepIndex: number;
              stepRole: string;
              proposal: { id: string; title: string; project?: { title: string } };
            }) => (
              <li key={r.id} className="p-4 space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-medium text-sm">{r.proposal.title}</span>
