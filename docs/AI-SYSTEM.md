...
- Live UI: **execution theater**, **document forge**, **regulatory forge**, **glitter performance cursor**, **Mission Pulse** analytics strip (`GET /api/platform-agent/missions/:id/pulse` + `getMissionPulse` voice tool), and optional **ArabClue Agent** Chrome extension (`extensions/arabclue-agent` v3) with **Smart Install / auto-update** in Mission Control (ZIP via `/api/platform-agent/extension/download`), remote config (`/api/platform-agent/extension/config`), Etimad scan + universal capture + copilot, offline capture queue with auto-retry, and tab/selection/screenshot uplink via `POST /api/platform-agent/extension/ingest`.
