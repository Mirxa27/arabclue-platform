...
- Live UI: **execution theater**, **document forge**, **regulatory forge**, **glitter performance cursor**, **Mission Pulse** analytics strip (`GET /api/platform-agent/missions/:id/pulse` + `getMissionPulse` voice tool), and optional **Chrome Voice Agent extension** (`extensions/arabclue-agent`) with **Smart Install / auto-update** in Mission Control (ZIP via `/api/platform-agent/extension/download`), an offline capture queue with auto-retry, and tab/selection/screenshot uplink via `POST /api/platform-agent/extension/ingest`.
